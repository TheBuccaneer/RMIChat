...
public class GUI extends Application
{
		...
		while (i++ <= 3)
		{
			...
			if (result.get().length() > 0 && result.get().length() < 20)
			{
				...
				if (this.isNicknameOkay(result.get()))
					...
					errorMessage = "Nickname schon vergeben. Bitte anderen auswählen.";
			} else
			{
				errorMessage = "Nickname muss zwischen 1 und 20 Zeichen lang sein.";
			}
			this.connectionError(errorMessage + " Versuch: " + (i - 1) + " von " + 3);
		}
		this.closeThis();
	}
	...
	private boolean isNicknameOkay(String s)
	{
		...
		{
			this.connectionError("Bei der Nicknameabfrage gab es ein Problem");
		}
		return b;
	}
	...
	private void initClient()
	{
		if (this.client == null)
			...
			{
				this.connectionError("RemoteException. Server noch nicht am Laufen?");
				this.closeThis();
			} catch (MalformedURLException | NotBoundException e)
			{
				this.connectionError("Fehler im Binding!");
				this.closeThis();
			}
	}
